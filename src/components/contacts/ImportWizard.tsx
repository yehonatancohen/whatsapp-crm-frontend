...
export function ImportWizard({ open, onClose, onImport, isImporting }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white border border-charcoal rounded-2xl p-4 sm:p-6 w-full max-w-md shadow-2xl">
        <h2 className="text-lg font-semibold text-charcoal mb-5">Import Contacts</h2>

        {!result ? (
          <>
            <div
              className="border-2 border-dashed border-charcoal rounded-xl p-8 text-center cursor-pointer hover:border-slate-600 transition-colors"
            ...
            >
              ...
              {file ? (
                ...
                <>
                  <p className="text-muted text-sm mb-1">Click to select a file</p>
                  <p className="text-faded text-xs">CSV or Excel (.xlsx, .xls)</p>
                </>
              )}
            </div>
            ...
            <div className="flex justify-end gap-3 mt-5">
              ...
              <button
                ...
                className="px-5 py-2 text-sm font-medium bg-accent hover:bg-accent disabled:bg-cream disabled:text-faded text-white rounded-lg transition-colors"
              >
          ...
          <>
            <div className="space-y-2 text-sm">
              ...
              {result.listName && (
                <div className="flex justify-between pt-1 border-t border-charcoal">
                  <span className="text-muted">Added to list</span>
            ...
            {result.errorDetails && result.errorDetails.length > 0 && (
              <div className="mt-4">
                ...
                <div className="max-h-40 overflow-y-auto bg-white border border-red-500/20 rounded-lg divide-y divide-slate-800">
                  {result.errorDetails.map((d: any, i: number) => (
                    <div key={i} className="px-3 py-2 text-xs">
                      <span className="text-faded">Row {d.row}</span>
            ...
            <div className="flex justify-end mt-5">
              <button
                onClick={handleClose}
                className="px-5 py-2 text-sm font-medium bg-accent hover:bg-accent text-white rounded-lg transition-colors"
              >
                Done
              </button>
            </div>
