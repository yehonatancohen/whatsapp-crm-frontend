...
export function ChatPage() {
  ...
  function handleImageFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      ...
      setPendingImage({ data: base64, mimeType: file.type, previewUrl: result });
      setImageCaption('');
    };
  ...
  async function handleSendImage() {
    if (!selectedChat || !pendingImage || sendImageMutation.isPending) return;
  ...
  const { data: selectedChatProfilePic } = useProfilePic(
              ...
              {filtered.map((chat) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-start gap-2 sm:gap-3 px-3 py-2.5 sm:px-4 sm:py-3 text-right transition-colors border-b border-border hover:bg-cream active:bg-cream-dark overflow-hidden ${isSelected ? 'bg-cream-dark' : ''}`}
                  >
        ...
        {selectedChat ? (
          <>
            ...
            {showAvatarModal && (
              ...
              >
                <div className="bg-white dark:bg-cream-dark border border-border rounded-xl p-4 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
                  {selectedChatProfilePic?.url ? (
                    <img src={selectedChatProfilePic.url} alt={selectedChat.name} className="w-full max-h-[70vh] object-contain rounded-lg" />
                  ) : (
                    <div className="w-full h-64 rounded-lg bg-cream-dark border border-border flex items-center justify-center text-muted">אין תמונת פרופיל זמינה</div>
                  )}
                </div>
              </div>
            )}
                    ...
                    return (
                      <div key={msg.id} className="w-full">
                        ...
                        >
                          ...
                          <div className={`msg-in relative max-w-[72%] sm:max-w-[75%] md:max-w-[72%] min-w-0 rounded-xl px-2.5 sm:px-3 py-1.5 shadow-sm text-[13.5px] sm:text-[14px] leading-[18px] sm:leading-[19px] overflow-hidden ${msg.fromMe ? sentBubble : recvBubble} ${showTail ? (msg.fromMe ? 'rounded-tr-sm' : 'rounded-tl-sm') : ''}`}>
                            ...
                            {msg.quotedMsg && (
                              <div className={`border-r-2 border-accent rounded px-2 py-1 mb-1.5 text-[11.5px] ${msg.fromMe ? 'bg-black/10 dark:bg-black/20' : 'bg-black/5 dark:bg-white/10'}`}>
                                <p className="font-semibold text-accent text-[10px] truncate leading-tight mb-0.5">{msg.quotedMsg.fromMe ? 'אתה' : (msg.quotedMsg.author?.replace('@c.us','') || 'נמען')}</p>
                                <p className="opacity-70 truncate leading-tight" dir="auto">{msg.quotedMsg.body || '[מדיה]'}</p>
                              </div>
                            )}
                              ...
                              {msg.fromMe && msg.ack != null && (
                                msg.ack === 0 ? (
                                  ...
                                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className={`w-[14px] h-[14px] ${msg.ack >= 3 ? 'text-blue-400' : 'opacity-50'}`}>
                                    {msg.ack >= 2 ? (
                                      <><polyline points="4 12.5 8.5 17 16.5 8" /><polyline points="10 12.5 14.5 17 22.5 8" /></>
                                    ) : (
                                      <polyline points="4 12 9.5 17 20 6" />
                                    )}
                                  </svg>
                                )
                              )}
            ...
            {/* Image preview panel */}
            {pendingImage && (
              <div className="px-3 pt-2 pb-0 bg-white dark:bg-cream-dark border-t border-border z-10 w-full">
                <div className="flex items-end gap-3 bg-cream border border-border rounded-xl p-2">
                  <img src={pendingImage.previewUrl} alt="תצוגה מקדימה" className="w-20 h-20 object-cover rounded-lg shrink-0" />
                  <div className="flex-1 min-w-0">
                    <input
                      ...
                      dir="auto"
                      className="w-full bg-white border border-border text-charcoal rounded-lg px-3 py-2 text-sm outline-none placeholder:text-muted focus:ring-1 focus:ring-accent/50"
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      type="button"
                      onClick={() => { setPendingImage(null); setImageCaption(''); }}
                      className="w-8 h-8 rounded-full bg-white border border-border flex items-center justify-center text-muted hover:text-red-500 transition-colors"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
                    </button>
                    <button
                      ...
                      className="w-10 h-10 rounded-full bg-accent flex items-center justify-center text-[#ffffff] hover:bg-accent-hover transition-colors disabled:opacity-50"
                    >
            ...
            {/* Input */}
            <form onSubmit={handleSend} className={`p-2 sm:p-3 bg-white dark:bg-cream-dark flex items-center gap-2 z-10 w-full ${replyTo || pendingImage ? '' : 'border-t border-border'}`}>
              {/* Image attach button */}
              {!isRecording && (
                <button
                  type="button"
                  ...
                  className="w-9 h-9 rounded-full bg-cream border border-border flex items-center justify-center text-muted hover:text-accent hover:border-accent/30 transition-colors shrink-0 disabled:opacity-40"
                  title="שלח תמונה"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4.5 h-4.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21 15 16 10 5 21" /></svg>
                </button>
              )}
